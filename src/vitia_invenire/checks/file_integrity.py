    ...
    def _verify_kernel_binaries(self, findings: list[Finding]) -> None:
        ...
        for binary in kernel_binaries:
            ...
            verify_result = run_ps(
                ...
                "$sha = if($hash){$hash.Hash}else{'ERROR'}; "
                "$st = if($sig){$sig.Status.ToString()}else{'Unknown'}; "
                "$sn = if($sig -and $sig.SignerCertificate){$sig.SignerCertificate.Subject}else{''}; "
                "$is = if($sig -and $sig.SignerCertificate){$sig.SignerCertificate.Issuer}else{''}; "
                "@{ Exists=$true; SHA256=$sha; Status=$st; Signer=$sn; Issuer=$is } "
                "} else { "
