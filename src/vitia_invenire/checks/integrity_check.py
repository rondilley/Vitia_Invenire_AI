    ...
    def _verify_kernel_binaries(self, findings: list[Finding]) -> None:
        ...
        for file_path, description in _CRITICAL_BINARIES:
            ...
            hash_result = run_ps(
                ...
                "$sn = if($sig.SignerCertificate){$sig.SignerCertificate.Subject}else{''}; "
                "$ts = if($sig.TimeStamperCertificate){$sig.TimeStamperCertificate.Subject}else{''}; "
                "@{ Path=$hash.Path; SHA256=$hash.Hash; SignatureStatus=$sig.Status.ToString(); Signer=$sn; TimeStamper=$ts } "
                "} else { "
